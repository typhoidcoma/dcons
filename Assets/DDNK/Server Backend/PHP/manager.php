...
if($cmd == "")
{
	...
	// Check to see if we have an account setup with the sent Unity PlayerPref
	$sql->select("accounts", "WHERE `user_id`='$user'");
	$row = $sql->fetch();
	...
	{
		$username = "Somebody";
		$newUser = createNewUser($user, $username);
		
		$sql->select("accounts", "WHERE `user_id`='".$newUser."'");
		$row = $sql->fetch();
	}
	...
	if($row["user_id"] != "")
	{
			...
			$sql->update("accounts", $upd, "WHERE `user_id`='".$auth."'");
