...
if($cmd == "")
{
	...
	{
		$username = "Somebody";
		$newUser = createNewUserWorldLoader($user, $username);
		
		$sql->select("accountsWorldLoader", "WHERE `user_id`='".$newUser."'");
		$row = $sql->fetch();
	}
	...
	if($row["user_id"] != "")
	{
			...
			$sql->update("accountsWorldLoader", $upd, "WHERE `user_id`='".$auth."'");
